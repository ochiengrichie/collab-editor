...
const navItems = [
  {
    label: "Dashboard",
    to: "/dashboard",
    isActive: ({ pathname, search }) => pathname === "/dashboard" && search === "",
    icon: (
  ...
  },
  {
    ...
    isActive: ({ pathname, search }) => pathname === "/dashboard" && search === "?view=my-docs",
    icon: (
      <svg viewBox="0 0 24 24" aria-hidden="true">
  ...
  },
  {
    ...
    isActive: ({ pathname, search }) => pathname === "/dashboard" && search === "?view=shared",
    icon: (
      <svg viewBox="0 0 24 24" aria-hidden="true">
      ...
      <nav className="sidebar-links">
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `sidebar-link${
                (item.isActive?.({ pathname: location.pathname, search: location.search }) ?? isActive)
                  ? " is-active"
                  : ""
              }`
            }
