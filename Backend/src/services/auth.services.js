//The Brain of Auth related operations: Registration, Login, Token generation 
//This file handles the core logic for user authentication, including secure password 
...
export const loginUser = async (req, res) => {
    ...
    const token = jwt.sign({ id: user.id, email: user.email }, JWT_SECRET, { expiresIn: '1h' });
    res.cookie('token', token, COOKIE_OPTIONS);
    return createResponse(res, true, { user: { id: user.id, email: user.email } }, null, 200);
    } catch (error) {
...
export const googleLogin = async (req, res) => {
  ...
  try {
    ...
    if (user){
      ...
      if (!user.google_sub) {
        await db.query('UPDATE users SET google_id = $1 , auth_provider=$2 WHERE id = $3', [goodleSub, 'google', user.id]);
      }
    } else {
      ...
      create = await db.query(
        'INSERT INTO users (email, google_id, auth_provider) VALUES ($1, $2, $3) RETURNING id, email',
        [email, goodleSub, 'google']
      );
      user = create.rows[0];
    }
