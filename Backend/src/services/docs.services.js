...
export const listUserDocuments = async (req, res) => {
  const userId = req.user.id;
    ...
    const docs = await db.query(
      `SELECT d.id, d.title, COALESCE(d.content, '') AS content, d.owner_id, dm.role FROM documents d 
...
export const renameDocument = async (req, res) => {
    ...
    const updatedDoc = await db.query(
      `UPDATE documents SET title = $1 WHERE id = $2 AND id IN (
       SELECT document_id FROM document_members WHERE user_id = $3
      ) RETURNING id, title, COALESCE(content, '') AS content, owner_id`,
